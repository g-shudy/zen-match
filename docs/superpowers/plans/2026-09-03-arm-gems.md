...
A beam gem's `arms` is only ever null on a cell that reached the engine without one; every
path that reads it must agree on the same fallback, so define it once:
...
Run: `node build.mjs --test && node --test tests/engine.test.js`
Expected: every engine test passes (19 original minus 1 deleted, plus 7 from Task 1, plus 13 new = 38).
...
Run: `npm test`
Expected: storage tests pass (9); engine and palette tests pass; total 50.
...
Expected: no hits in CSS or TS. (`index.html` still has a legend sample with the space-separated classes `special-line horizontal`, which this grep does not match; Task 5 rewrites it.)

- [ ] **Step 5: Typecheck, build, and run the suite**

Run: `npm run typecheck && npm test`
Expected: clean; 50 tests pass.
...
Run: `npm test`
Expected: 53 tests pass (50 plus the 3 new).
...
Expected: clean, 53 tests pass, version `2.1.0`.
